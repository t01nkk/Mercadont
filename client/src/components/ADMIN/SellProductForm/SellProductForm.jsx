...
export default function SellProductForm() {
    ...
    const [data, setData] = useState({
        ...
        price: '',
        description: '',
        image: 'https://static.zara.net/photos///2022/V/1/1/p/6469/910/060/2/w/850/6469910060_6_4_1.jpg?ts=1651057109246',
        status: 'inactive',
        stock: '',
        categories: [],
    });
    ...
    function validator(input) {
        ...
        if (
            !expression.priceExpression.test(input.price) &&
            input.price !== ''
        ) {
        ...
        if (
            !expression.stockExpression.test(input.stock) &&
            input.stock !== ''
        ) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const { name, price, description, image, status, stock, categories } =
            data;
        if (
            !errors.name &&
            !errors.price &&
            ...
            try {
                await axios.post(
                    `${process.env.REACT_APP_DOMAIN}/product/create`,
                    {
                        name: name,
    ...
    return (
        <div className="container-login">
            <div className="sellProductCard">
                <p className="sellProduct-title">
                    {t('adminSellProduct.postProduct')}
                </p>
                ...
                >
                    <div className="sell-inputs-container">
                        ...
                        {errors.name && (
                            <p className="error-input-edit">{errors.name}</p>
                        )}
                        ...
                        {errors.price && (
                            <p className="error-input-edit">{errors.price}</p>
                        )}
                        ...
                        <div className="sellproduct-status">
                            ...
                            >
                                <option value="active">
                                    {t('adminEditProduct.active')}
                                </option>
                                <option value="inactive">
                                    {t('adminEditProduct.inactive')}
                                </option>
                            </select>
                        </div>
                        <input
                        ...
                        />
                        {errors.stock && (
                            <p className="error-input-edit">{errors.stock}</p>
                        )}
                        <select
                        ...
                        <div className="sell-select-categories-del">
                            {data.categories?.map((category, i) => (
                                <div key={i} className="edit-cat-delete">
                                    ...
                                    <button
                                        onClick={(event) =>
                                            handleDeleteCat(category, event)
                                        }
